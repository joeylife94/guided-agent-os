...
하지만 다음 AI 품질 지표는 아직 체계적으로 검증하지 않는다.
...
- BGE-M3 또는 동급 multilingual embedding model
...
Enterprise legacy system을 가정한 read-only lookup API.
...
- Approval permits allowlisted execution
    
- Unauthorized Tool block
    
- Restricted action block
...
외부 검토자가 5~10분 안에 다음을 이해할 수 있어야 한다.
